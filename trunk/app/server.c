...
/*
   When server A comes up:
   A joins P2P network by following steps:
   1. Read peers.lst and find predecessor B and succesor C
   2. A connects to B and C and sends P2P_JOIN_REQUEST
   3. A recieves P2P_JOIN_RESPONSE from B and C
   4. A close connection to B but keep connection to C
   B does the following
   1. B sends back P2P_JOIN_RESPONSE containg all user data in its primary range
   2. B closes connection to its previous successor
   3. B connects to A's TCP port and keeps the connection
   C does the following:
   1. C modifies its primary and backup range
   2. C finds which players should be primarily stored at A
   3. C sends back P2P_JOIN_RESPONSE containg players from previous step
   */
enum {
	FAULT_TYPE_NONE = 0,
	FAULT_LOSSY_LINK,
...
int main(int argc, char* argv[]){
	...
	while( (c=getopt( argc,argv,"u:t:f:"))!=-1){
		switch(c){
			...
			case 'f': s_fault = atoi(optarg);break;
			default:
	...
	int nonlocalip = *(struct in_addr*)(hp->h_addr_list[i]);
	char * ipchar = inet_ntoa(ip);
			...
			if (FD_ISSET(i,&readfds)){
				...
				if (i==udplistener){ 
					...
					if (read_bytes <= 0){ 
						...
						if(read_bytes <= 0){
							/*
TODO: Project 2 Part B
Check if the other end is a successor
if Yes:
Read the peers.lst
Find backup
Send BKUP_REQUEST when neccessary
*/
							...
							if(bufferd->flag == HEADER){
								...
								if(hdr->msgtype == LOGIN_REQUEST){ // LOGIN REQUEST
									if (FD_ISSET(i,&login)){ // Is he typing in login again?
										...
										/* Figure out whether the source host is a successor or predecessor
										   If successor:
										   Find all the userdata in its primary range
										   Construct the P2P_JOIN_RESPONSE with the data
										   Send it
										   Close connection with the current successor
										   Connects to the new successor
										   else if a predescessor
										   Modify the primary and backup range
										...
										if( pred_si->p2p_id == jr_payload->requestp2p_id ){
											// Modify its primary and backup range

											// primary->high and backup->low stay the same
											primary->low = jr_payload->requestp2p_id+1;
											backup->high = jr_payload->requestp2p_id;
											...
											for(p = primarylist->head; p; p=p->next){
												...
												if(curp->p2p_id >= backup->low && curp->p2p_id <= backup->high){ // If in range
													...
													memcpy(userdata+offset,htonl(curp->hp),4);    offset += 4;
													memcpy(userdata+offset,htonl(curp->exp),4);   offset += 4;
											...
											// DONE HANDLING FOR REQUEST FROM SUCCESSOR


										}

										// ** Handling request from my new succ **
										if ( succ_si->p2p_id == requestp2p_id ){
											// I AM the predessesor of the new guy
											// send back p2p_join_response containing all user data in my primary range
											// close connection to previous successor
											// I now connect to my new successor and keep that connection
											...
											// Need to CLOSE connection with previous succsessor (how do i obtain my PREV_SUCC???)
											close(succ_sock);
											FD_CLR(succ_sock,&master); // remove from the master set
											succ_sock = i;

											// Keep connection with new guy

											// DONE HANDLING FOR REQUEST FROM PREDECESSOR
										...
										int i;
										for(i=0; i<total; i++){
											// TODO: Create function constructPlayer
											...
											if(isInRange(player->p2p_id,backup)){
												// Need to make sure that there are no duplicate
